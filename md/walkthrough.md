...
## 10. Queue 상태 조회 회귀 테스트 추가 및 검증 (완료)
**작업 일시**: 2026-03-11  
**작업 내용**:
- 상태 조회 핵심 시나리오를 단위 테스트로 명시하고, 변경 로직이 요구사항대로 동작하는지 검증함.

- **주요 변경 (Review - Why)**:
  - **QueueService 상태 조회 테스트 신설**
    - 파일: `services-queue/src/test/kotlin/com/yourssu/morupark/queue/business/QueueServiceStatusTest.kt`
    - 검증 케이스:
      1. ZSET 존재 -> `WAITING`(rank/estimatedWaitSeconds 포함)
      2. ZSET 미존재 + `PROCESSING` -> `PROCESSING`
      3. ZSET 미존재 + `SUCCESS` -> `SUCCESS`
      4. ZSET 미존재 + `FAILED:msg` -> `FAILED`(message 포함)
      5. ZSET 미존재 + `COMPLETED` -> `SUCCESS` alias
      6. ZSET/상태/user 모두 없음 -> `InvalidWaitingTokenException`(401 경로)
      7. ZSET 미존재 + 상태 누락 + user 존재 -> `PROCESSING` 보정
    - 이유: 이번 이슈의 핵심 실패 패턴(짧은 상태 공백 구간)과 호환성 케이스를 테스트로 고정해 회귀를 방지하기 위함.

  - **빌드/테스트 실행 결과**
    - 실행: `services-queue` 경로에서 `.\gradlew.bat test`
    - 결과: `BUILD SUCCESSFUL`
    - 참고: 샌드박스 환경 특성상 초기 1회 Gradle 다운로드 권한 이슈가 있어 권한 승인 후 재실행하여 성공 확인함.
