# VPC 네트워크 생성
resource "google_compute_network" "morupark_vpc" {
  name                    = "morupark-vpc-prod"
  auto_create_subnetworks = false
}

# Public 서브넷 생성
resource "google_compute_subnetwork" "public_subnet" {
  name          = "morupark-subnet-prod"
...
resource "google_compute_firewall" "allow_lb" {
  name    = "allow-lb-ingress-prod"
  network = google_compute_network.morupark_vpc.name
  allow {
    protocol = "tcp"
    ports    = ["80", "443"]
  }
